...
class RejectedCashesRepo {
  static async find() {
    const { rows } = await pool.query(
      'SELECT * FROM rejectedCashes JOIN users ON users.id = rejectedCashes.userId;',
    );
    return toCamelCase(rows);
  }
  static async insert(paymentCashUrl, userId) {
    await pool.query(
      'INSERT INTO rejectedCashes(paymentCashUrl, userId) VALUES ($1, $2) RETURNING *; ',
      [paymentCashUrl, userId],
    );
  }
