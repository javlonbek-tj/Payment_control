...
const findByCategories = async (course, mentor, paymentstatus, dateFrom, dateTo, history, limit, offset) => {
  ...
  if (course && mentor && paymentstatus && dateFrom && dateTo && history) {
    ...
    const totalUsers = parseInt(countRows.rows.length);
  ...
  if (course && mentor && !paymentstatus && !dateFrom && !dateTo && history) {
    ...
    const countRows = await pool.query('SELECT * FROM users WHERE course = $1 AND mentor = $2 AND role = $3 AND history = $4;', [
      course,
      mentor,
      'user',
      history,
